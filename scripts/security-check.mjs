...
const rbac=read('src/lib/rbac.ts');for(const marker of ['companyId:session.companyId,active:true','role:{companyId:session.companyId}','role:{companyId:s.companyId'])if(!rbac.includes(marker))issues.push(`Live authorization marker missing: ${marker}`);checks.push('live-user-session-authorization','company-scoped-permission-resolution');
const criticalScope={
