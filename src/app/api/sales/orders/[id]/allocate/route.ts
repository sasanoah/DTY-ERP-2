...
export async function POST(_:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    const result=await prisma.$transaction(async tx=>{
      ...
      const plannedByLot=new Map<string,number>();const initialReservedByLot=new Map<string,number>();
      for(const line of order.lines){
        ...
        const free=lots.reduce((a,l)=>a+Math.max(0,Number(l.availableQtyKg)-Number(l.reservedQtyKg)-Number(plannedByLot.get(l.id)||0)),0);
        if(free+0.001<need)throw Object.assign(new Error(`مخزون المنتج المفرج غير كافٍ. عجز ${Math.ceil(need-free)} كجم`),{status:409});
        for(const lot of lots){const alreadyPlanned=Number(plannedByLot.get(lot.id)||0);const available=Math.max(0,Number(lot.availableQtyKg)-Number(lot.reservedQtyKg)-alreadyPlanned);if(available<=0)continue;const take=Math.min(need,available);plan.push({lineId:line.id,lotId:lot.id,qty:take});plannedByLot.set(lot.id,alreadyPlanned+take);if(!initialReservedByLot.has(lot.id))initialReservedByLot.set(lot.id,Number(lot.reservedQtyKg));need-=take;if(need<=0.001)break}
      }
      const reservedByLot=new Map(initialReservedByLot);
      for(const p of plan){
        const expectedReserved=Number(reservedByLot.get(p.lotId)||0);
        const u=await tx.finishedLot.updateMany({where:{id:p.lotId,qcStatus:'RELEASED',reservedQtyKg:expectedReserved,availableQtyKg:{gte:expectedReserved+p.qty}},data:{reservedQtyKg:{increment:p.qty}}});
        if(u.count!==1)throw Object.assign(new Error('تغير رصيد المنتج أثناء التخصيص؛ أعد المحاولة'),{status:409});
        reservedByLot.set(p.lotId,expectedReserved+p.qty);
