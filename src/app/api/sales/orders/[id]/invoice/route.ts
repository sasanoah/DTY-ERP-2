...
export async function POST(_:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    const result=await prisma.$transaction(async tx=>{
      ...
      const claimed=await tx.salesOrder.updateMany({where:{id,plantId:s.plantId,status:'DELIVERED'},data:{status:'INVOICED'}});
      if(claimed.count!==1)throw Object.assign(new Error('تم إصدار فاتورة لهذا الأمر بالفعل'),{status:409});
