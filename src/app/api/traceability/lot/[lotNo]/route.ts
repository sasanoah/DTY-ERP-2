...
export async function GET(_:Request,{params}:{params:Promise<{lotNo:string}>}){try{const s=await requirePermission('traceability.read');if(!s.plantId)throw Object.assign(new Error('PLANT_REQUIRED'),{status:400}); const {lotNo}=await params;
 const raw=await prisma.inventoryLot.findFirst({where:{lotNo,warehouse:{plantId:s.plantId},material:{companyId:s.companyId}},include:{material:true,supplier:true,movements:true,materialIssues:{include:{productionRun:{include:{productionOrder:{include:{product:true,machine:true}},finishedLots:{include:{deliveryLines:{include:{delivery:{include:{salesOrder:{include:{customer:true}}}}}}}}}}}}}});
