...
export type SessionPayload={userId:string;companyId:string;plantId?:string;username:string;roles:string[];sessionVersion:number;exp:number};
...
export function verifySessionToken(token?:string|null):SessionPayload|null{if(!token)return null;const [body,sig]=token.split('.');if(!body||!sig)return null;const a=Buffer.from(sig);const b=Buffer.from(sign(body));if(a.length!==b.length||!timingSafeEqual(a,b))return null;try{const p=JSON.parse(unb64(body)) as SessionPayload;return p.exp>Date.now()&&Number.isInteger(p.sessionVersion)&&p.sessionVersion>=0?p:null}catch{return null}}
